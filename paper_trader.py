...
def load_fix17_runtime_module():
    """
    GitHub Actions runtime for the formally recovered FIX17/FIX13 sizing
    and entry-time margin functions.

    IMPORTANT:
      - FIX17_OFFICIAL_FULL_SOURCE.py is still SHA-verified.
      - The frozen official source is NOT executed here, because its
        historical Colab/Google Drive recovery chain is not a live-runtime
        dependency and fails under GitHub Actions.
      - The four functions below are the formally recovered implementations
        used by the FIX17 live execution layer.
      - ENTRY / EXIT signal logic is not implemented or changed here.
    """
    ...
    # Formal inherited constants required by the recovered functions.
    LOT_SIZE = 100
    LONG_CAP_PER_STOCK = 2000000.0
    SUBSTITUTE_HAIRCUT = 0.8
    ...
    def calc_entry_time_position_state(
        ...
        """
        Formal recovered position-state calculation.

        The historical backtest version obtains the previous mark through
        get_last_close_before().  The live paper-state adapter supplies a
        PrevMark value for already-open positions.  If it is unavailable,
        the formal fallback remains EntryPrice.
        """
    ...
    runtime.calc_entry_time_position_state = (
        calc_entry_time_position_state
    )
    runtime.calc_long_dynamic_max_positions = (
        calc_long_dynamic_max_positions
    )
    runtime.calc_margin_capped_qty = (
        calc_margin_capped_qty
    )
    runtime.calc_target_qty = (
        calc_target_qty
    )
...
def calculate_fix17_order(
    ...
    if isinstance(capped_qty, dict):
        capped_qty = capped_qty["AllowedQty"]
...
def get_fix17_position_state(
    ...
    for p in active_positions(
        ...
        broker_active[
            ...
            # Live equivalent of the already-stored position mark.
            # The formal function still falls back to EntryPrice when
            # this is unavailable/non-finite.
            "PrevMark":
                p.get(
                    "mark_price",
                    float("nan"),
